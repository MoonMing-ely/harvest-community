...
> v0.5.2 · 终端、供应链与独立发布加固

## 1. 系统目标

Harvest Community 保留原有的本地优先、故障恢复、结构化输出和可修订边界，同时把单用户固定六主线改为可复用的用户建模流程。
...
最终确认后删除原始问卷、测试答案、测试报告和网络摘要，不创建正式日报。正式日报生成成功后直接保存并删除 pending，用户通过 `harvest revise YYYY-MM-DD` 后续校正；重新建档以当前画像版本为基线，确认前不替换当前画像，也不改写历史日志。第五份日报前只临时保存自然语言修订意见，校准完成后清除。模型请求设置 `store=false`，但请求内容仍会发送给用户选择的模型供应商。原始请求与响应仅可通过 `doctor --api-test --details` 在当前进程中查看，不会持久化；API Key 和 Authorization 始终被排除。
...
Prompt 将指令与 JSON 数据分离，并明确把 JSON 内字符串视为不可信内容；严格 Schema 限制模型输出形状。项目记忆建议会通过原有名称和状态规则校验，自动应用的结果或跳过原因会显示在终端。语义提示注入无法被完全消除，但它不能直接执行命令或绕过终端净化。

## 5. 跨平台层

- 配置目录由 `platformdirs` 解析。
- API Key 通过 `keyring` 接入系统凭据库；Linux 后端使用 Secret Service。
- Reminder 模块按平台生成 systemd、launchd 或 Task Scheduler 配置，统一调用无模型副作用的 `harvest notify`。命令只在本地检查目标日期是否已有日报和是否仍处于三小时补提醒窗口；Linux/macOS/Windows 分别投递 Freedesktop、Notification Center 和 Toast 非模态通知。
...
自动测试覆盖严格 Schema、动态渲染、旧数据迁移、画像版本、差异计算、主线匹配、pending 生命周期、日报自动保存与后续修订、项目记忆自动更新、完整 onboarding、不限轮次修改、回答纠错、重新建档、AI 等待状态、第五份日报触发、跨平台提醒定义、Provider 错误、密钥脱敏、配置与系统任务注入、终端控制字符、Rich markup、旧 Markdown 清理、哈希锁和 CI SHA 固定。
